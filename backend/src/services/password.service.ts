...
export async function requestPasswordReset(email: string, ipAddress?: string) {
  ...
  await db.transaction(async (trx) => {
    await trx('PasswordResetTokens').where({ UserID: user.UserID }).whereNull('UsedAt').update({ UsedAt: trx.fn.now() });
...
export async function resetPassword(token: string, newPassword: string) {
  return db.transaction(async (trx) => {
    ...
    await trx('Users').where({ UserID: reset.UserID, IsActive: true }).update({ PasswordHash: await bcrypt.hash(newPassword, 12),
      MustChangePassword: false, TokenVersion: trx.raw('ISNULL(TokenVersion, 0) + 1'), UpdatedAt: trx.fn.now() });
    await trx('PasswordResetTokens').where({ ResetID: reset.ResetID }).update({ UsedAt: trx.fn.now() });
    await trx('AuthSessions').where({ UserID: reset.UserID }).whereNull('RevokedAt').update({ RevokedAt: trx.fn.now() });
...
export async function changePassword(userId: number, currentPassword: string, newPassword: string, actor: WorkflowActor) {
  return db.transaction(async (trx) => {
    ...
    await trx('Users').where({ UserID: userId }).update({ PasswordHash: await bcrypt.hash(newPassword, 12),
      MustChangePassword: false, TokenVersion: Number(user.TokenVersion ?? 0) + 1, UpdatedAt: trx.fn.now() });
