...
export default function PipelineDashboard({ role }: { role: PipelineRole }) {
  ...
  const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [limit, setLimit] = useState(10); const debouncedQuery = useDebouncedValue(query);
  useEffect(() => { let active = true; setLoading(true); Promise.all([adminApi.getPipeline(role, page, limit, debouncedQuery.trim()), adminApi.getUsers(`role=${config[role].staffRole}&active=true&page=1&limit=100`)])
    .then(([pipeline, users]) => { if (!active) return; setRows(pipeline.data.data ?? []); setTotal(pipeline.data.total ?? 0); setWorkload(pipeline.data.workload ?? []);
      setStaff(users.data?.users ?? []); setError(''); })
    .catch((cause) => active && setError(cause instanceof Error ? cause.message : 'Unable to load pipeline.'))
    .finally(() => active && setLoading(false)); return () => { active = false; }; }, [debouncedQuery, limit, role, page]);
    ...
    <div className="grid gap-5 lg:grid-cols-12"><aside className="overflow-hidden rounded-2xl border bg-white shadow-sm lg:col-span-4"><div className="border-b px-5 py-4"><h2 className="text-xs font-bold uppercase tracking-widest text-slate-500">Team capacity</h2>
      ...
      <section className="relative overflow-hidden rounded-2xl border bg-white shadow-sm lg:col-span-8">{loading ? <div className="absolute inset-0 z-10 grid place-items-center bg-white/70"><LoadingSpinner size="md" /></div> : null}
        <div className="flex items-center gap-3 border-b p-4"><div><h2 className="text-sm font-bold">Action queue</h2><p className="text-[10px] text-slate-500">Oldest stage entry first · {total} total</p></div>
          <div className="relative ml-auto"><Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-slate-400" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search full queue"
            className="h-9 w-52 rounded-xl border bg-slate-50 pl-8 pr-3 text-xs outline-none focus:border-slate-400" /></div></div>
        <div className="min-h-64 divide-y">{rows.map((row) => { const hours = age(row); const isOverdue = hours >= config[role].sla;
          ...
          {!rows.length && !loading ? <p className="p-12 text-center text-sm text-slate-400">Queue is empty for this view.</p> : null}</div></section></div>
    {total > 0 && <DataPagination page={page} limit={limit} total={total} loading={loading} pageSizes={[10, 25, 50]} onPageChange={setPage} onLimitChange={(value) => { setLimit(value); setPage(1); }} />}
  </div>;
}
