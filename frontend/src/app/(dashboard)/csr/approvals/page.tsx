...
export default function ApprovalsPage() {
  ...
  const [page, setPage] = useState(1); const [limit, setLimit] = useState(12); const debouncedQuery = useDebouncedValue(query, 160);
  ...
  const load = useCallback(() => { setLoading(true); const params = new URLSearchParams({ page: String(page), limit: String(limit) }); if (debouncedQuery) params.set('search', debouncedQuery);
    return Promise.all([screeningApi.getPendingCSR(params.toString()), screeningApi.getCSRStats()])
    .then(([queue, summary]) => { setRows(queue.data?.applications ?? []); setPendingTotal(Number(queue.data?.pagination?.total ?? summary.data?.pending ?? 0)); setError(''); })
  ...
  return <div className="space-y-6"><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-widest text-emerald-700">Independent sponsor control</p>
    ...
    <div className="flex items-center gap-2 rounded-xl border bg-white px-3 py-2"><Search size={15} className="text-slate-400" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search name, scholarship or APP ID" className="w-full text-sm outline-none" /></div>
    {error ? <p role="alert" className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">{error}</p> : null}
    <section className="overflow-hidden rounded-2xl border border-pink-100 bg-white shadow-sm"><div className="flex items-center justify-between border-b bg-pink-50/50 px-5 py-3"><span className="flex items-center gap-2 text-sm font-semibold text-emerald-800"><ShieldCheck size={16} />Showing {rows.length} of {pendingTotal} pending</span>
      <span className="text-[10px] text-slate-500">Oldest first · no bulk approval</span></div><div className="divide-y">{rows.map((row) => { const waiting = age(row.stageEnteredAt); return <div key={row.applicationId}
        ...
        {!rows.length && !loading ? <p className="p-12 text-center text-sm text-slate-400">No applications awaiting sponsor approval.</p> : null}{loading ? <p className="p-12 text-center text-sm text-slate-400">Loading approval queue…</p> : null}</div></section>
