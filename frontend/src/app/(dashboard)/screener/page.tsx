...
export default function ScreenerDashboard() {
  ...
  const [stats, setStats] = useState<ScreenerStats>(emptyStats); const [initialSearch, setInitialSearch] = useState('');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [queue, summary, decisions] = await Promise.all([
        screeningApi.getPendingScreening(), screeningApi.getStats(), screeningApi.getHistory(),
      ]);
      setRows(queue.data ?? []); setStats(summary.data ?? emptyStats); setHistory(decisions.data ?? []);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Screening workload could not be loaded.'); }
    finally { setLoading(false); }
  }, []);
  useEffect(() => { setInitialSearch(new URLSearchParams(window.location.search).get('search') ?? ''); void load(); }, [load]);
  ...
  return <div className="screener-theme flex min-h-screen flex-col"><ScreenerHeader /><main className="mx-auto w-full max-w-[1400px] space-y-6 px-4 py-6 sm:px-6 sm:py-8">
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_330px]"><ScreeningQueue rows={rows} userId={user?.userId} loading={loading} initialSearch={initialSearch} /><aside className="space-y-5"><ScreeningActivity rows={history} /><ScreeningControlsGuide /></aside></div>
  </main></div>;
}
