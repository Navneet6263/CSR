...
import { Download, Search, ShieldAlert, Wallet } from 'lucide-react';
...
export default function PaymentQueue() {
  ...
  const [query, setQuery] = useState(''); const debouncedQuery = useDebouncedValue(query, 160); const [page, setPage] = useState(1); const [limit, setLimit] = useState(12); const [total, setTotal] = useState(0); const [queueAmount, setQueueAmount] = useState(0); const [loading, setLoading] = useState(true);
  const load = useCallback(() => { setLoading(true); const params = new URLSearchParams({ page: String(page), limit: String(limit) }); if (debouncedQuery) params.set('search', debouncedQuery);
    adminApi.getPaymentQueue(params.toString()).then((response) => { setRows((response.data?.applications ?? []).map(mapRow)); setTotal(response.data?.pagination?.total ?? 0); setQueueAmount(response.data?.summary?.amount ?? 0); setError(''); }).catch((reason: Error) => setError(reason.message)).finally(() => setLoading(false)); }, [debouncedQuery, limit, page]);
  useEffect(() => { load(); }, [load]);
  ...
  return <div className="space-y-5"><div className="flex items-end justify-between"><div><p className="text-[10px] uppercase tracking-widest text-slate-400">Operations · Read only</p>
    ...
    <div className="flex items-center gap-2 rounded-xl border bg-white px-3 py-2"><Search className="h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search student, scholarship or APP ID" className="w-full text-sm outline-none" /></div>
    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">{[
      { label: 'In Queue', value: String(total), icon: Wallet }, { label: 'Queue Value', value: `₹${queueAmount.toLocaleString('en-IN')}` },
