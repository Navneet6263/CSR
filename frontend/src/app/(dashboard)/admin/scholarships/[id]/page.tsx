...
import { ArrowLeft, Calendar, Download, FilePenLine, GraduationCap, Pause, Play, Wallet } from 'lucide-react';
...
export default function ManageScholarship() {
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    ...
    <header className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-lg bg-slate-900 text-white"><GraduationCap className="h-4 w-4" /></div><div><h1 className="text-2xl font-semibold">{scholarship.Name}</h1><p className="text-xs text-slate-500">{scholarship.SponsorName} · {scholarship.Status} · Content: {scholarship.ContentStatus || 'Not started'} · Closes {new Date(scholarship.ApplicationCloseDate).toLocaleDateString('en-IN')}</p></div></div><div className="flex flex-wrap gap-2"><button onClick={exportSummary} className="inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-xs"><Download className="h-3.5 w-3.5" />Export</button><button title={!contentPublished && scholarship.Status !== 'Active' ? 'Approve and publish the content first' : undefined} disabled={saving || scholarship.Status === 'Closed' || (scholarship.Status !== 'Active' && !contentPublished)} onClick={toggleStatus} className="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 disabled:opacity-40">{scholarship.Status === 'Active' ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}{scholarship.Status === 'Active' ? 'Pause' : 'Activate'}</button><Link href={`/admin/scholarships/${id}/rules`} className="rounded-lg border px-3 py-2 text-xs">Edit Rules</Link><Link href={`/admin/scholarships/${id}/content`} className="inline-flex items-center gap-1.5 rounded-lg bg-slate-900 px-3 py-2 text-xs text-white"><FilePenLine className="h-3.5 w-3.5" />Content Builder</Link></div></header>
