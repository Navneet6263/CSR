...
export default function SupportTickets() {
  ...
  const load = useCallback(async () => { setLoading(true); setError('');
    try { const response = await supportApi.tickets(status, mine, query.trim()); setRows(response.data ?? []); }
  ...
  const totals = useMemo(() => ({ urgent: rows.filter((row) => row.Priority === 'Urgent' && row.Status !== 'Resolved').length,
    unassigned: rows.filter((row) => !row.AssignedTo && row.Status !== 'Resolved').length,
    overdue: rows.filter((row) => row.DueAt && new Date(row.DueAt) < new Date() && row.Status !== 'Resolved').length }), [rows]);
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5"><header><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-amber-600">Resolution queue</p>
    ...
    <section className="flex flex-col gap-3 rounded-2xl border bg-white p-4 shadow-sm lg:flex-row lg:items-center"><div className="relative flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
      <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search requester, subject or email" className="h-9 w-full rounded-xl border bg-slate-50 pl-9 pr-3 text-sm outline-none focus:border-amber-400" /></div>
      <div className="flex gap-1 overflow-x-auto">{statuses.map((item) => <button key={item} onClick={() => setStatus(item)} className={`shrink-0 rounded-lg px-2.5 py-2 text-[10px] font-bold ${status === item ? 'bg-slate-950 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>{item === 'InProgress' ? 'In progress' : item === 'WaitingOnUser' ? 'Waiting on user' : item}</button>)}</div>
      <button onClick={() => setMine((value) => !value)} className={`rounded-lg px-3 py-2 text-xs font-bold ${mine ? 'bg-amber-400 text-slate-950' : 'border text-slate-600'}`}>Assigned to me</button></section>
